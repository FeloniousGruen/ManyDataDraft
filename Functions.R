...
approx_posterior <- function(fit_exp, # model fit objective from fit_causl: MLE and its variance from the experimental data 
  ...
  FI <- fit_exp$OI + eta * fit_obs$OI # fisher information
  ...
  if (use_sandwich == TRUE){ # If using the sandwich estimator of variance
    ...
    theta_hat <- invFI %*% (fit_exp$OI %*% theta_e + eta *fit_obs$OI %*% theta_o)
  }
...
approx_posterior_multiple_datasets <- function(fit_exp, fit_obs_list, eta_list, 
  ...
  if (!is.matrix(fit_exp$OI)) {
    stop("fit_exp$OI must be a matrix")
  }
  ...
  for (i in seq_along(fit_obs_list)) {
    ...
    if (!is.matrix(fit_obs_i$OI) ) {
      stop(paste("fit_obs_list[[", i, "]]$OI  must be a matrix.", sep = ""))
    }
    
    # Check matrix dimensions match fit_exp
    if (!all(dim(fit_obs_i$OI) == dim(fit_exp$OI))) {
      stop(paste("Dimension mismatch: fit_obs_list[[", i, "]]$OI does not match fit_exp$OI.", sep = ""))
    }
  }
  ...
  if (use_sandwich == T){
    ...
    theta_hat <- invFI %*% (fit_exp$OI %*% theta_e) 
    for (i in seq_along(fit_obs_list)) {
      eta_i <- eta_list[i]
      fit_obs_i <- fit_obs_list[[i]]
      theta_o_i <- fit_obs_i$par
      theta_hat <- theta_hat + invFI %*% (eta_i * fit_obs_i$OI %*% theta_o_i)
    }
  }
...
calculate_elpd <- function(samples, data, mm, msks,
  ...
  # Convert family to proper causl format
  family_converted <- lapply(causl::family_list(family), function(f) f())
...
new_method_optim <- function(fit_exp, fit_obs_list, A_empirical) {
  ...
  n_e <- nrow(fit_exp$mm)
  V <-  matrix(solve.default(fit_exp$OI), nrow = nrow(fit_exp$OI), ncol = ncol(fit_exp$OI)) *n_e
  ...
  for (i in seq_along(fit_obs_list)) {
    ...
    if (!is.matrix(fit_obs_i$OI) || nrow(fit_obs_i$OI) != ncol(fit_obs_i$OI)) {
      stop(sprintf("fit_obs_list[[%d]]$OI is not a square matrix.", i))
    }
    
    # Compute W_i as the inverse of FI scaled by n_o_i
    W_i <- solve(fit_obs_i$OI) * n_o_i
...
generate.data.list.HTE <- function(n.r, n.o, seed,
  ...
  family <- list(c(1), c(5, 5,5, 1), 1, 1)        # used for *all* arms
  
  ## ----- 2.  Build the single randomised arm ----------------------------- ##
  forms_exp_d <- list(
    c(X1 ~ 1),
    list(A ~ 1, X3 ~ 1, X2 ~ 1, U1 ~1),                # treatment randomised
    Y ~ A + X3 + A:X3 +X2+ U1,
    ~ 1
  )
  ...
  for (k in seq_len(n_datasets)) {
    ...
    forms_obs_d <- list(
      c(X1 ~ 1),
      list(A ~ X1 + X2 + U1, X3 ~ 1, X2 ~ 1, U1 ~ 1),
      Y ~ A + X3 + A:X3 + X2 + U1,
      ~ 1
    )
    ...
    if (U.diff) {
      pars_obs_d <- list(
        ...
        X2 = list(beta = 0,          phi = 1),
        X3 = list(beta = 0),
        U1 = list(beta = U1.mean.shift,          phi = 1),
...
sim_func <- function(omega,seed, RBesT_inc = T, atmle_inc = T, 
  ...
  if (confounding_fn_inc){
    ...
    # # Check the actual class and dimensions
    # class(rct_data$A)
    # dim(rct_data$A)
    # is.vector(rct_data$A)
    
    # Fix by converting to a plain vector
    rct_data$A <- as.vector(rct_data$A)
    obs_data$A <- as.vector(obs_data$A)
    
    # Then run again
    intHTE_obj <- IntegrativeHTEcf::IntHTEcf(
      data.rct = rct_data,
      data.rwe = obs_data,
      outcome.method = "gam",
      cfName = c("X1", "X2", "X3"),
